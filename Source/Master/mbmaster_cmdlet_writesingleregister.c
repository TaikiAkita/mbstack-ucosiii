...
*
* File      : MBMASTER_CMDLET_WRITESINGLEREGISTER.C
* Version   : V1.0.320
* By        : Ji WenCong
*********************************************************************************************************
*/
...
#define MBMASTER_CMDLET_WRITEHREG_SOURCE
...
static void MBMaster_CmdLet_FC06_ReqHdl(
    CPU_INT08U   slave,
    void        *p_request,
    CPU_INT08U  *p_buffer,
...
static void MBMaster_CmdLet_FC06_ResHdl(
    CPU_INT08U   slave,
    void        *p_request,
    void        *p_response,
    void        *p_responsearg,
...
MBMASTER_CMDLET  g_MBMaster_CmdLet_FC06 = {
    .cbRequestHandler = MBMaster_CmdLet_FC06_ReqHdl,
    .cbResponseHandler = MBMaster_CmdLet_FC06_ResHdl
};


/*
*********************************************************************************************************
*                                  MBMaster_CmdLet_FC06_ReqHdl()
*
* Description : Make request frame for "Write Single Register (0x06)" command.
*
...
*               (2) 'p_request' must points to a 'MBMASTER_CMDLET_WRITESINGLEREGISTER_REQUEST' object.
*********************************************************************************************************
*/
...
static void MBMaster_CmdLet_FC06_ReqHdl(
    ...
    MBMASTER_CMDLET_WRITESINGLEREGISTER_REQUEST     *request;
    ...
    *p_error = MB_ERROR_NONE;
    ...
    /*  Type cast.  */
    request = (MBMASTER_CMDLET_WRITESINGLEREGISTER_REQUEST*)p_request;
    ...
    p_frame->functionCode = MB_FNCODE_WRITESINGLEREGISTER;
...
/*
*********************************************************************************************************
*                                  MBMaster_CmdLet_FC06_ResHdl()
*
* Description : Handle response frame of "Write Single Register (0x06)" command.
*
* Argument(s) : (1) slave                 Slave address.
*               (2) p_request             Pointer to the request object.
...
*
* Note(s)     : (1) Interrupts are assumed to be disabled when calling this function.
*               (2) 'p_request' must point to a 'MBMASTER_CMDLET_WRITESINGLEREGISTER_REQUEST' object.
*               (3) 'p_response' must point to a 'MBMASTER_CMDLET_WRITESINGLEREGISTER_RESPONSE' object.
*********************************************************************************************************
*/
...
static void MBMaster_CmdLet_FC06_ResHdl(
    ...
    MBMASTER_CMDLET_WRITESINGLEREGISTER_REQUEST    *request;
    MBMASTER_CMDLET_WRITESINGLEREGISTER_RESPONSE   *response;
    ...
    *p_error = MB_ERROR_NONE;
    ...
    /*  Type cast.  */
    request  = (MBMASTER_CMDLET_WRITESINGLEREGISTER_REQUEST *)p_request;
    response = (MBMASTER_CMDLET_WRITESINGLEREGISTER_RESPONSE*)p_response;
    ...
    if (p_frame->functionCode == MB_FNCODE_WRITESINGLEREGISTER) {
        ...
        /*  Read and check the written register value.  */
        field16 = MBBufFetcher_ReadUInt16BE(
            &(fetcher),
            p_error
        );
        ...
        if (field16 != request->hregValue) {
            *p_error = MB_ERROR_MASTER_RXINVALIDFORMAT;
            return;
        }

        /*  Invoke the completion callback.  */
        if (response->cbComplete != (MBMASTER_HREGWRITTEN_CB)0) {
            response->cbComplete(
                request->hregAddress,
                request->hregValue,
                p_responsearg,
    ...
    } else if (p_frame->functionCode == (CPU_INT08U)(MB_FNCODE_WRITESINGLEREGISTER + (CPU_INT08U)0x80U)) {
