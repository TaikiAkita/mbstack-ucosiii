...
*
* File      : MBMASTER_CMDLET_MASKWRITEREGISTER.C
* Version   : V1.0.320
* By        : Ji WenCong
*********************************************************************************************************
*/
...
#define MBMASTER_CMDLET_MASKWRITEHREG_SOURCE
...
static void MBMaster_CmdLet_FC16_ReqHdl(
    CPU_INT08U   slave,
    void        *p_request,
    CPU_INT08U  *p_buffer,
...
static void MBMaster_CmdLet_FC16_ResHdl(
    CPU_INT08U   slave,
    void        *p_request,
    void        *p_response,
    void        *p_responsearg,
...
MBMASTER_CMDLET  g_MBMaster_CmdLet_FC16 = {
    .cbRequestHandler = MBMaster_CmdLet_FC16_ReqHdl,
    .cbResponseHandler = MBMaster_CmdLet_FC16_ResHdl
};


/*
*********************************************************************************************************
*                                  MBMaster_CmdLet_FC16_ReqHdl()
*
* Description : Make request frame for "Mask Write Register (0x16)" command.
*
...
*               (2) 'p_request' must points to a 'MBMASTER_CMDLET_MASKWRITEREGISTER_REQUEST' object.
*********************************************************************************************************
*/
...
static void MBMaster_CmdLet_FC16_ReqHdl(
    ...
    MBMASTER_CMDLET_MASKWRITEREGISTER_REQUEST     *request;
    ...
    *p_error = MB_ERROR_NONE;
    ...
    /*  Type cast.  */
    request = (MBMASTER_CMDLET_MASKWRITEREGISTER_REQUEST*)p_request;
    ...
    p_frame->functionCode = MB_FNCODE_MASKWRITEREGISTER;
...
/*
*********************************************************************************************************
*                                  MBMaster_CmdLet_FC16_ResHdl()
*
* Description : Handle response frame of "Mask Write Register (0x16)" command.
*
* Argument(s) : (1) slave                 Slave address.
*               (2) p_request             Pointer to the request object.
...
*
* Note(s)     : (1) Interrupts are assumed to be disabled when calling this function.
*               (2) 'p_request' must point to a 'MBMASTER_CMDLET_MASKWRITEREGISTER_REQUEST' object.
*               (3) 'p_response' must point to a 'MBMASTER_CMDLET_MASKWRITEREGISTER_RESPONSE' object.
*********************************************************************************************************
*/
...
static void MBMaster_CmdLet_FC16_ResHdl(
    ...
    MBMASTER_CMDLET_MASKWRITEREGISTER_REQUEST    *request;
    MBMASTER_CMDLET_MASKWRITEREGISTER_RESPONSE   *response;
    ...
    *p_error = MB_ERROR_NONE;
    ...
    /*  Type cast.  */
    request  = (MBMASTER_CMDLET_MASKWRITEREGISTER_REQUEST *)p_request;
    response = (MBMASTER_CMDLET_MASKWRITEREGISTER_RESPONSE*)p_response;
    ...
    if (p_frame->functionCode == MB_FNCODE_MASKWRITEREGISTER) {
        /*  Read and check the written register address.  */
        ...
        if (field16 != request->hregAddress) {
            *p_error = MB_ERROR_MASTER_RXINVALIDFORMAT;
            return;
        }

        /*  Read and check the 'And' mask.  */
        field16 = MBBufFetcher_ReadUInt16BE(
            &(fetcher),
            p_error
        );
        ...
        if (field16 != request->hregAndMask) {
            *p_error = MB_ERROR_MASTER_RXINVALIDFORMAT;
            return;
        }

        /*  Read and check the 'Or' mask.  */
        field16 = MBBufFetcher_ReadUInt16BE(
            &(fetcher),
            p_error
        );
        ...
        if (field16 != request->hregOrMask) {
            *p_error = MB_ERROR_MASTER_RXINVALIDFORMAT;
            return;
        }

        /*  Invoke the completion callback.  */
        if (response->cbComplete != (MBMASTER_HREGMASKWRITTEN_CB)0) {
            response->cbComplete(
                request->hregAddress,
                request->hregAndMask,
                request->hregOrMask,
    ...
    } else if (p_frame->functionCode == (CPU_INT08U)(MB_FNCODE_MASKWRITEREGISTER + (CPU_INT08U)0x80U)) {
