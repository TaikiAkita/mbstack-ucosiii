...
*
* File      : MBMASTER_CMDLET_WRITEMULTIPLEREGISTERS.C
* Version   : V1.0.320
* By        : Ji WenCong
*********************************************************************************************************
*/
...
#define MBMASTER_CMDLET_WRITEHREGS_SOURCE
...
static void MBMaster_CmdLet_FC10_ReqHdl(
    CPU_INT08U   slave,
    void        *p_request,
    CPU_INT08U  *p_buffer,
...
static void MBMaster_CmdLet_FC10_ResHdl(
    CPU_INT08U   slave,
    void        *p_request,
    void        *p_response,
    void        *p_responsearg,
...
MBMASTER_CMDLET  g_MBMaster_CmdLet_FC10 = {
    .cbRequestHandler = MBMaster_CmdLet_FC10_ReqHdl,
    .cbResponseHandler = MBMaster_CmdLet_FC10_ResHdl
};


/*
*********************************************************************************************************
*                                  MBMaster_CmdLet_FC10_ReqHdl()
*
* Description : Make request frame for "Write Multiple Registers (0x10)" command.
*
...
*               (2) 'p_request' must points to a 'MBMASTER_CMDLET_WRITEMULTIPLEREGISTERS_REQUEST' object.
*********************************************************************************************************
*/
...
static void MBMaster_CmdLet_FC10_ReqHdl(
    ...
    MBMASTER_CMDLET_WRITEMULTIPLEREGISTERS_REQUEST  *request;

    CPU_INT16U                                       hregQuantity;
    CPU_INT16U                                      *hregValuePtr;
    ...
    *p_error = MB_ERROR_NONE;
    ...
    /*  Type cast.  */
    request = (MBMASTER_CMDLET_WRITEMULTIPLEREGISTERS_REQUEST*)p_request;
    ...
    if (
        ...
        (request->hregQuantity  > (CPU_INT16U)0x007BU)
    ) {
    ...
    MBBufEmitter_WriteUInt8(
        &(emitter),
        (CPU_INT08U)((request->hregQuantity) <<= (CPU_INT16U)1U),
        p_error
    );
    switch (*p_error) {
        case MB_ERROR_NONE:
    ...
    hregValuePtr = request->hregValues;
    while (hregQuantity != (CPU_INT16U)0U) {
        /*  Write the value of the holding register.  */
        MBBufEmitter_WriteUInt16BE(
            &(emitter),
            *(hregValuePtr),
            p_error
    ...
    p_frame->functionCode = MB_FNCODE_WRITEMULTIPLEREGISTERS;
...
/*
*********************************************************************************************************
*                                  MBMaster_CmdLet_FC10_ResHdl()
*
* Description : Handle response frame of "Write Multiple Registers (0x10)" command.
*
* Argument(s) : (1) slave                 Slave address.
*               (2) p_request             Pointer to the request object.
...
*
* Note(s)     : (1) Interrupts are assumed to be disabled when calling this function.
*               (2) 'p_request' must point to a 'MBMASTER_CMDLET_WRITEMULTIPLEREGISTERS_REQUEST' object.
*               (3) 'p_response' must point to a 'MBMASTER_CMDLET_WRITEMULTIPLEREGISTERS_RESPONSE' object.
*********************************************************************************************************
*/
...
static void MBMaster_CmdLet_FC10_ResHdl(
    ...
    MBMASTER_CMDLET_WRITEMULTIPLEREGISTERS_REQUEST   *request;
    MBMASTER_CMDLET_WRITEMULTIPLEREGISTERS_RESPONSE  *response;
    ...
    *p_error = MB_ERROR_NONE;
    ...
    /*  Type cast.  */
    request  = (MBMASTER_CMDLET_WRITEMULTIPLEREGISTERS_REQUEST *)p_request;
    response = (MBMASTER_CMDLET_WRITEMULTIPLEREGISTERS_RESPONSE*)p_response;
    ...
    if (p_frame->functionCode == MB_FNCODE_WRITEMULTIPLEREGISTERS) {
        /*  Read and check the holding register start address.  */
        ...
        if (field16 != request->hregStartAddress) {
            *p_error = MB_ERROR_MASTER_RXINVALIDFORMAT;
            return;
        }
        /*  Read and check the holding register quantity.  */
        field16 = MBBufFetcher_ReadUInt16BE(
            &(fetcher),
            p_error
        );
        ...
        if (field16 != request->hregQuantity) {
            *p_error = MB_ERROR_MASTER_RXINVALIDFORMAT;
            return;
        }

        /*  Notify the upper application that all holding register values has been written.  */
        if (response->cbComplete != (MBMASTER_HREGSWRITTEN_CB)0) {
            response->cbComplete(
                request->hregStartAddress,
                request->hregQuantity,
                p_responsearg, 
    ...
    } else if (p_frame->functionCode == (CPU_INT08U)(MB_FNCODE_WRITEMULTIPLEREGISTERS + (CPU_INT08U)0x80U)) {
