...
*
* File      : MBSLAVE_CMDLET_READINPUTREGISTERS.C
* Version   : V1.0.320
* By        : Ji WenCong
*********************************************************************************************************
*/
...
#define MBSLAVE_CMDLET_READIREG_SOURCE
...
void MBSlave_CmdLet_ReadInputRegisters(
    ...
    *p_error = MB_ERROR_NONE;
    ...
    /*  Type cast.  */
    cmdlet_ctx = (MBSLAVE_READINPUTREGISTERS_CTX*)p_cmdlet_ctx;
    ...
    iregStartAddress = MBBufFetcher_ReadUInt16BE(
        &(fetcher),
        p_error
    );
    if (*p_error != MB_ERROR_NONE) {
    ...
    iregQuantity = MBBufFetcher_ReadUInt16BE(
        &(fetcher),
        p_error
    );
    if (*p_error != MB_ERROR_NONE) {
    ...
    if (iregQuantity == (CPU_INT16U)0x0000U || iregQuantity > (CPU_INT16U)0x007DU) {
        goto MBSLAVE_RDIREG_CATCH_INVALIDVALUE;
    }

    /*  Validate the start address.  */
    validity = cmdlet_ctx->cbValidateInputReg(
        iregStartAddress, 
        cmdlet_ctx->cbArg, 
        p_error
    );
    ...
    if (!validity) {
        goto MBSLAVE_RDIREG_CATCH_INVALIDADDR;
    }

    /*  Validate the end address.  */
    iregEndAddress = (CPU_INT16U)(iregStartAddress + iregQuantity - (CPU_INT16U)1U);
    if (iregEndAddress < iregStartAddress) {
    ...
    MBBufEmitter_WriteUInt8(
        &(emitter),
        (CPU_INT08U)(iregQuantity << (CPU_INT16U)1U),
        p_error
    );
    if (*p_error != MB_ERROR_NONE) {
    ...
    /*  Process the request.  */
    iregCurrentAddress = iregStartAddress;
    while (iregQuantity != (CPU_INT16U)0U) {
        /*  Read register value.  */
        iregValue = cmdlet_ctx->cbReadInputReg(
            iregCurrentAddress,
            cmdlet_ctx->cbArg,
            p_error
        );
    ...
    *p_response_fncode = request_fncode;
    ...
    goto MBSLAVE_RDIREG_FINALLY;

MBSLAVE_RDIREG_CATCH_INVALIDVALUE:
    ec = MB_APUEC_ILLEGALDATAVALUE;
    goto MBSLAVE_RDIREG_CATCH_FINALLY;

MBSLAVE_RDIREG_CATCH_INVALIDADDR:
    ec = MB_APUEC_ILLEGALDATAADDRESS;

MBSLAVE_RDIREG_CATCH_FINALLY:
    /*  Write the function code.  */
