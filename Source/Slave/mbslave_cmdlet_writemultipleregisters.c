...
*
* File      : MBSLAVE_CMDLET_WRITEMULTIPLEREGISTERS.C
* Version   : V1.0.320
* By        : Ji WenCong
*********************************************************************************************************
*/
...
#define MBSLAVE_CMDLET_WRITEHREG_SOURCE
...
/*
*********************************************************************************************************
*                                    MBSlave_CmdLet_WriteMultipleRegisters()
*
* Description : Command implementation of "Write Multiple Registers (0x10)".
*
...
*               (2) The 'p_cmdlet_ctx' pointer MUST point to a MBSLAVE_WRITEMULTIPLEREGISTERS_CTX; object.
*********************************************************************************************************
*/

void MBSlave_CmdLet_WriteMultipleRegisters(
    ...
    MBSLAVE_WRITEMULTIPLEREGISTERS_CTX *cmdlet_ctx;
    ...
    *p_error = MB_ERROR_NONE;
    ...
    /*  Type cast.  */
    cmdlet_ctx = (MBSLAVE_WRITEMULTIPLEREGISTERS_CTX*)p_cmdlet_ctx;
    ...
    /*  Read the byte count.  */
    inByteCount = MBBufFetcher_ReadUInt8(
        &(fetcher),
        p_error
    );
    if (*p_error != MB_ERROR_NONE) {
    ...
    if (hregQuantity == (CPU_INT16U)0x0000U || hregQuantity > (CPU_INT16U)0x007BU) {
        goto MBSLAVE_WRHREG_CATCH_INVALIDVALUE;
    }
    ...
    if (!validity) {
        goto MBSLAVE_WRHREG_CATCH_INVALIDADDR;
    }
    ...
    if (inByteCount != (CPU_INT08U)(hregQuantity << ((CPU_INT16U)1U))) {
        goto MBSLAVE_WRHREG_CATCH_INVALIDVALUE;
    }
    ...
    *p_response_fncode = request_fncode;
    ...
    goto MBSLAVE_WRHREG_FINALLY;

MBSLAVE_WRHREG_CATCH_INVALIDVALUE:
    ec = MB_APUEC_ILLEGALDATAVALUE;
    goto MBSLAVE_WRHREG_CATCH_FINALLY;

MBSLAVE_WRHREG_CATCH_INVALIDADDR:
    ec = MB_APUEC_ILLEGALDATAADDRESS;

MBSLAVE_WRHREG_CATCH_FINALLY:
    /*  Write the function code.  */
